...
class App:
    def __init__(self):
        root = tk.Tk()
        root.title('Sonic CV Demo')

        self.win = win = tk.PanedWindow(
        ...
        self.demos_lb = demos_lb = tk.Listbox(
            left, yscrollcommand=scrollbar.set)
        ...
        with open('samples_list','rb') as f:
        ...
        self.text = text = ScrolledText(right, font=(
            'arial', 12, 'normal'), width=30, wrap='word')
    ...
    def match_text(self, pattern, tag_proc, regexp=True):
        ...
        while True:
            match_index = text.search(
                pattern, 'matchPos', count=count, regexp=regexp, stopindex='end')
            if not match_index:
                break
            end_index = text.index("%s+%sc" % (match_index, count.get()))
            text.mark_set('matchPos', end_index)
            if callable(tag_proc):
                tag_proc(match_index, end_index,
                         text.get(match_index, end_index))
            else:
